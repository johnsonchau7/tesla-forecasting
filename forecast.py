...
class holt:
    ...
    def summary(self):
        ...
        print(' alpha (level) {0:.4f} ({1:.4f})'.format(self.alpha, self.se[0]))
        print(' beta (trend)  {0:.4f} ({1:.4f})'.format(self.beta, self.se[1]))
        if self.damped:
            print(' phi (damping) {0:.4f} ({1:.4f})'.format(self.phi, self.se[2]))
        print(' \n In-sample fit:')
        print(' MSE               {0:.4f}'.format(mse))
        if loglik>0:
            print(' Log-likelihood    {0:.4f}'.format(loglik))
        else: 
            print(' Log-likelihood   {0:.4f}'.format(loglik))
        print(' AIC               {0:.4f}'.format(aic))
        print(' BIC               {0:.4f}'.format(bic))
        self.aic=aic
        self.bic=bic
